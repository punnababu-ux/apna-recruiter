...
import { Badge, Button, Separator } from "@apna/design-system"
...
export function JobCard({
  ...
  return (
    <div 
      className={cn(
        "flex flex-col gap-5 rounded-xl border border-border bg-card p-5 sm:p-6 transition-all hover:border-border/80 hover:shadow-sm dark:hover:shadow-none dark:hover:bg-muted/10",
        className
      )}
      {...props}
    >
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Button variant="outline" onClick={onSave} className="h-9 w-9 p-0 sm:w-auto sm:px-4 shrink-0" aria-label="Save Job">
            <BookmarkPlus className="h-4 w-4 sm:mr-2" />
            <span className="hidden sm:inline">Save</span>
          </Button>
          <Button variant="default" onClick={onApply} className="h-9 px-6 shrink-0">
            Apply Now
          </Button>
        </div>
      </div>
